...
# SQL
    ...
    if "conn" not in streamlit.session_state:
        streamlit.session_state["conn"] = None


    if not streamlit.session_state["logged_in"]:
        streamlit.write("Enter your SQL Server login details:")
    ...
    if streamlit.button("Connect to SQL Server"):
        try:
            conn = pyodbc.connect(
                f"Driver={Driver};Server={Server};Database={Database};UID={UserID};PWD={Password};TrustServerCertificate=yes"
            )
            streamlit.session_state["logged_in"] = True
            streamlit.session_state["conn"] = conn
            streamlit.success("Connected to SQL Server successfully!")
            streamlit.rerun()  # Refresh to show the logged-in interface
        except Exception as e:
            streamlit.error(f"Connection failed: {e}")
            
    if streamlit.session_state["logged_in"]:
        streamlit.success("You are logged in!")
