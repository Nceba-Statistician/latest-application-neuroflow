        ...
        if all_records is not None:
            ...
            if x_axis and y_axis:
                try:
                    ...
                    chart_name = streamlit.text_input("", placeholder="Name this chart")
                    if streamlit.button(f"save {chart_name}", key=col1_dash):
                        if chart_name:
                            ...
                            streamlit.success(f"✅ You have successfully saved {chart_name}! - check saved charts at {save_path_root}")
                        else:
...
with col4_dash:
    ...
    try:
        ...
        if all_records is not None:
            ...
            chart_type = streamlit.selectbox("Select Chart Type:", ["", "Scatter Plot", "Bar Chart", "Line Chart", "Pie Chart", "Stacked Bar Chart"], key=col4_dash)
            x_axis = streamlit.selectbox("Select X-axis:", [""] + all_columns, key=col4_dash)
            y_axis = streamlit.selectbox("Select Y-axis:", [""] + all_columns, key=col4_dash)
            color_option = streamlit.selectbox("Optional Color Field:", [None] + all_columns, key=col4_dash)
            if x_axis and y_axis:
                try:
                    fig, ax = pyplot.subplots(figsize=(6, 4))
                    if chart_type == "Scatter Plot":
